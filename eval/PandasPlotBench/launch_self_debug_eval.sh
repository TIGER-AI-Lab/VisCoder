...
CONFIG="configs/config_self_debug_viscoder.yaml" # e.g., config_self_debug_baseline.yaml, config_self_debug_viscoder.yaml
LIMIT=None
...
CONFIG_SUFFIX=${CONFIG_BASENAME#config_}
MODE=$(echo "$CONFIG_SUFFIX" | rev | cut -d'_' -f2- | rev)
SUBMODE=$(echo "$CONFIG_SUFFIX" | rev | cut -d'_' -f1 | rev)
